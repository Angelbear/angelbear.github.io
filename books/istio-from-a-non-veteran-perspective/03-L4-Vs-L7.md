...
<img src="./03-http-imbalanced-route.svg"
     alt="HTTP Imbalanced Trffic"
     style="display: block; width: 59%; margin: auto" />
     
This process is just fine for normal release requrement, which you will have sort of the live / canary mechanism to test the new rollout. But obviously this is not perfect:

* The granularity of the ramp up / down can not be precisely controlled. Take the above example, we have 3 replicas of old pods, then even if we only ramp up 1 new pod at first, we will immediately have 25% connections connecting to the new Pod. It will be not possible to ramp up only 1% of the traffic with under this situation.
* Traffic split 